...
/ (Project Root)
├── .github/workflows/      <- CI/CD templates (semantic-release, npm)
├── .releaserc.js           <- Release config
├── TODO.md                 <- The generated/synced markdown file
├── package.json            <- Enhanced with Commitizen & Semantic Release
├── server.ts               <- Main Express instance combining backend & frontend
└── src/
    ├── backend/
    │   ├── controllers/    <- TodoController.js translated to modern ESM/TS
    │   ├── middleware/     <- TodosMiddleware.js
    │   ├── models/         <- JSON file readers/writers
    │   └── utils/          <- IdIncrementor, DateTime, ErrorLogger
    ├── data/
    │   └── todos.json      <- The JSON store
    └── frontend/           <- Replacing HTML templates
        ├── components/     <- Modals, Tables, Forms
        └── pages/          <- Dashboard (index), Settings (types, statuses)