...
import { Settings, X, Terminal, Circle, Plus, Trash2, Power, AlertCircle, Upload, Package, FileCode, RefreshCw, Download, HardDrive, Check, Ban, Loader2, Play, Eye, Skull, Sparkles } from 'lucide-react';
            ...
            <div className="space-y-2 pt-4">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                     ...
                     <button 
                        ...
                        disabled={isScanning}
                        className="flex items-center gap-1.5 text-[10px] font-bold uppercase text-slate-400 hover:text-cyan-600 transition-colors"
                        title="Scan disk for new plugins"
                     >
                        <RefreshCw size={12} className={isScanning ? 'animate-spin text-cyan-500' : ''} />
                ...
                {repoPlugins.length === 0 ? (
                    ...
                    repoPlugins.map(repoPlugin => {
                        ...
                        return (
                            <div key={repoPlugin.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800 rounded-lg group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                                            ...
                                            {!isInstalled && (
                                                <span className="text-[9px] bg-fuchsia-100 dark:bg-fuchsia-950 text-fuchsia-600 dark:text-fuchsia-400 px-1.5 py-0.5 rounded border border-fuchsia-200 dark:border-fuchsia-900 flex items-center gap-1">
                ...
                {shutdownState === 'stopping' && (
                    <>
                        <div className="relative">
                            ...
                            <Skull className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-red-500/50 mt-[-16px]" size={24} />
                        </div>
